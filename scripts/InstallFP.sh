...
# Official behavior (InstallAP.sh + boot_patch.sh): kernel ALWAYS gets a REAL
# superkey. Keyless (zeroed root_superkey) breaks apd/su auth -> app says
# "root unavailable" even with patched=true. Manager app itself needs NO
# key entry (signature-auth) - key below is for kernel/apd, shown + saved.
SKEY=""
for d in /sdcard /data/media/0 /data/media /external_sd; do
  if [ -f "$d/FolkPatch-key.txt" ]; then
    ...
    if [ -n "$SKEY" ]; then
      case "$SKEY" in
        Ap*) ui_print "- Reusing saved key from $d/FolkPatch-key.txt"; break ;;
        *) SKEY="" ;;
      esac
    fi
  fi
...
if [ -z "$SKEY" ]; then
  ...
  SKEY="Ap$R"
  ui_print "- New superkey: $SKEY (kernel needs REAL key, zeroed=unavailable)"
fi
...
if [ "$RC" -ne 0 ]; then
  ui_print "- NOTE: -S failed ($RC), trying legacy -s ..."
  kp_run -p -i kernel-origin -s "$SKEY" -k "$KPIMG" -o kernel >"$WORK/patch.log" 2>&1
  RC=$?
  if [ "$RC" -eq 0 ]; then KEYMODE="skey"; fi
fi
if [ "$RC" -ne 0 ]; then
  ui_print "- NOTE: -s failed ($RC), trying combined -s+-S ..."
...
ui_print "- Key mode OK: $KEYMODE ($SKEY)"
...
# Kernel MUST hold our REAL key now (zeroed = keyless bug = unavailable).
_VL=$(run_grep -i "root_superkey" "$WORK/verify.log" 2>/dev/null | head -n 1)
if [ -n "$_VL" ]; then
  ui_print "- $_VL"
  case "$_VL" in
    *000000000000*) abort "root_superkey ZEROED - keyless patch happened, app will say unavailable. Send log!" ;;
  esac
else
